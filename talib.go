...
// upperband, middleband = BBandsUp(close, timeperiod=5, nbdevup=2, matype=0)
//
// inNbDevUp: Deviation multiplier for upper band
// inMAType: Type of Moving Average
//
...
// middleband, lowerband = BBandsDn(close, timeperiod=5, nbdevdn=2, matype=0)
//
// inNbDevDn: Deviation multiplier for lower band
// inMAType: Type of Moving Average
//
...
// Abs - Vector ABS
func Abs(inReal []float64) []float64 {
	outReal := make([]float64, len(inReal))
	for i := 0; i < len(inReal); i++ {
		outReal[i] = math.Abs(inReal[i])
	}
	return outReal
}

// Pow - Vector Pow returns x**y, the base-x exponential of y.
func Pow(inReal []float64, y float64) []float64 {
	outReal := make([]float64, len(inReal))
	for i := 0; i < len(inReal); i++ {
		outReal[i] = math.Pow(inReal[i], y)
	}
	return outReal
}
